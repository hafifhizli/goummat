...
class Anggota extends CI_Controller
{
    ...
    function karyawan_perhari()
    {
        $tanggal = $this->input->post('tanggal');
        $tanggal1 = explode('-', $tanggal);
        $this->load->library('f_pdf');
        $data["hari"] =  $tanggal1[0];
        $data["bulan"] =  $tanggal1[1];
        $data["tahun"] =  $tanggal1[2];
        $data['karyawan'] = $this->m_data->ambil_harian($tanggal)->result();
        $this->load->view('admin/pdf/v_karyawan_perhari', $data); //memanggil view 
    }
    function karyawan_perbulan()
    {
        $tanggal = $this->input->post('tanggal');
        $anggota = $this->input->post('karyawan');
        $tanggal1 = explode('-', $tanggal);
        $bulan =  $tanggal1[0];
        $tahun =  $tanggal1[1];
        $this->load->library('f_pdf');
        $data["bulan"] =  $tanggal1[0];
        $data["tahun"] =  $tanggal1[1];
        if ($_REQUEST['karyawan'] == 'all') {
            $data['karyawan'] = $this->m_data->ambil_perbulan_all($bulan, $tahun)->result();
            $this->load->view('admin/pdf/v_karyawan_perbulan', $data); //memanggil view 
        } else {
            $data['karyawan'] = $this->m_data->ambil_perbulan($anggota, $bulan, $tahun)->result();
            $this->load->view('admin/pdf/v_karyawan_perbulan_selected', $data); //memanggil view 
        }
    }

    function karyawan_pertahun()
    {
        $tanggal = $this->input->post('tanggal');
        $karyawan = $this->input->post('karyawan');
        $this->load->library('f_pdf');
        $data['tahun'] = $this->input->post('tanggal');
        if ($_REQUEST['karyawan'] == 'all') {
            $data['karyawan'] = $this->m_data->ambil_pertahun_all($tanggal)->result();
            $this->load->view('admin/pdf/v_karyawan_pertahun', $data); //memanggil view 
        } else {
            $data['karyawan'] = $this->m_data->ambil_pertahun($tanggal, $karyawan)->result();
            $this->load->view('admin/pdf/v_karyawan_pertahun_selected', $data); //memanggil view 
        }
    }

    function rekap_event($id)
    {
        $this->load->library('f_pdf');
        $data['event'] = $this->m_data->get_rekap_judul($id)->result();
        $data['karyawan'] = $this->m_data->get_rekap_event($id)->result();
        $this->load->view('admin/pdf/v_karyawan_event', $data); //memanggil view 
    }
}
